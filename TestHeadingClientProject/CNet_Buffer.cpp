...
void CNet_Buffer::commit( uint64_t _length )
{
	...
	printf( "getHeader : %lld / %lld [", m_data, m_dataSize );
	Util::PrintMem( ( char* )m_data, _length );
	printf( "] \n" );
}
...
bool CNet_Buffer::get_buffer( char** _buffer, uint64_t* _length )
{
	if( DEFAULT_SOCKET_BUFFER_LENGTH > m_dataSize )
	{
		...
		*_buffer = m_data + m_dataSize;
		printf( "get_buffer return length : %lld / [%llX / %llX] \n", *_length, m_data, *_buffer );
		return true;
	}
...
Header* CNet_Buffer::get_data()
{
	...
	if( m_dataSize >= sizeof( Header ) )
	{
		...
		printf( "getHeader : %lld / %lld [", getHeader->type, getHeader->length );
		Util::PrintMem( ( char* )getHeader, sizeof( Header ) );
		printf( "] \n");
		if( m_dataSize >= getHeader->length )
		{
			switch( getHeader->type )
			{
				...
				case 1000:
					...
					break;
				default:
					printf( "parse fail!! : %llX/%llX/%lld [", getHeader, m_data, ( ( uint64_t )getHeader ) - ( ( uint64_t )m_data ) );
					Util::PrintMem( ( char* )getHeader, sizeof( Header ) );
					printf("]\n");
			}
		}
